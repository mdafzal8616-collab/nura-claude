...
## 2026-09-24 — Home richness pass: reversing the decluttering, without losing the modal/collapse/teaser work

The owner's newest instruction explicitly reversed the direction of the two prior Home passes (decluttering, then the gold theme): don't turn Home into a single "next action" card, don't remove things to get whitespace — make it genuinely richer, with better hierarchy instead of less content. Read as: keep everything the decluttering pass moved out (the prayer-time modal, the collapsible Priority card, the three-tab nav) but stop treating "Today Progress" and "what matters today" as one-line teasers, and stop leaving Hamdard/Library undiscoverable from Home. Every addition below reuses data sources that already existed (`todayItems`/`progressCalc`, the Next Step Engine, `quranProgress`, Plan My Day, money goals, habits) — nothing new was invented to fill space.

**What was added to Home** (`js/app.js`, `index.html`, `css/style.css`):
- A one-line, real day-context sentence under the motivation line (`renderDayContext`, from a per-stage sentence map plus "there's still time" / "everything's done" appended from real open-item counts — never a fixed message).
- A compact horizontal "day rail" (`renderDayRail`) — one small dot per stage (✓ past, gold current, dim future) across the whole Fajr-to-Tahajjud day, sitting above the existing detailed day-flow accordion rather than replacing it. Found and fixed a real bug while testing: the rail didn't auto-scroll to the current stage, so on a 10-stage day the highlighted "you are here" dot could be scrolled off-screen by default — added `scrollIntoView` on the current item.
- A real "Today's Priorities" list (`todaysPrioritiesList`/`renderTodaysPriorities`), 2-4 rows, each sourced from a real module (the legacy Today's Priority if pending, up to 2 pending Plan My Day tasks, the Deen priority if undone, real weekly money saved, one open habit) — never hardcoded text, and the card hides itself entirely when there's nothing real to show.
- `progressCalc`/`todayItems` gained a `bucket` field (Deen/Focus/Health/Personal, mapped from each item's real category) so Today Progress could show a real category breakdown inline instead of only Deen/Dunya — rewritten to a percent + count + small grid, still on Home (not hidden behind a click), still feeding the same weekly snapshot.
- A real Hamdard insight card (`renderHamdardInsight`), distinct from the existing "Right Now" card — one of three sentences chosen from today's actual done/open counts, never a canned line, hidden when there's no data yet.
- A contextual Library card (`renderLibraryContext`) that only appears from Maghrib onward, showing the real "continue reading" surah/ayah when one exists or a generic Qur'an nudge otherwise — not a permanent fixture.
- A 4-button Quick Actions row (Start Focus, Log Salah, Add Task, Ask Hamdard), each wired to the same real handlers those actions already use elsewhere (`startDuniyaQuickAction`, `openQuickCapture`, Plan My Day, the Hamdard tab).

**Fixed during testing**: the priority-row title and subtitle were both inline `<span>`s with no line-break rule, so they visually ran together ("Read a little Qur'anYour Deen priority") — added `display:flex; flex-direction:column` on the row's text wrapper.

**Deliberately unchanged**: the Next Step ("Right Now") card, the Next Salah card, the collapsible legacy Priority card, the prayer-time modal, and the day-flow accordion — all still exactly where the decluttering pass left them, so nothing is duplicated between the new cards and the old ones (checked explicitly: Salah info only ever appears in Right Now + Next Salah + day-flow, same as before).

**Tested**: true fresh state (cleared localStorage, sessionStorage, and the IndexedDB backup — the app has an existing auto-restore-from-backup feature that otherwise silently repopulates "cleared" storage, which is correct behavior, not a bug, but had to be worked around to see a genuinely empty state); no-prayer-times Home (new cards correctly absent/hidden, no console errors); Asr and Night day stages with real prayer times set; marking a prayer done and watching Right Now, Today Progress, and the Hamdard card all update from the same real numbers with no page reload; the Today's Priorities "Open" button and the Library card's "Open"/"Resume" button both navigating correctly; all 4 Quick Action buttons: Start Focus lands on the real "Prepare to study" picker step, Log Salah opens the real Quick Capture modal, Add Task opens Plan My Day, Ask Hamdard opens the real (non-chat, scripted) Hamdard screen; bottom nav stayed visible and correctly highlighted throughout. Checked at both desktop pane width and a true 375×812 mobile viewport — quick actions, day rail, and every new card fit without overflow. No console errors at any point beyond a pre-existing, unrelated browser warning (about unset time inputs in an off-screen modal instance) that reproduces even on a totally untouched fresh load, confirmed not caused by this change. Not tested on a physical phone.
...
**Navigation collapsed from 5 tabs to 3**: Life (was Today, still view id `home`), Library (new), Hamdard. Deen, Dunya and Progress are no longer destinations of their own — every one of their screens (`view-sunnah`, `view-duniya-*`, `view-progress`) is unchanged internally and still reachable, just through Library instead of a tab, so zero stored data or working screen logic was touched. `setActiveView`'s nav-highlight logic now maps all of `sunnah`/`duniya*`/`progress` to the Library tab; `initHeaderAvatars`/`MAIN_VIEWS` updated to include `library`. Profile stays behind the avatar, unchanged.

**Library** (`LIBRARY_GROUPS`, `renderLibrary`): one search box plus seven small life-need groups (Pray, Read & Learn, Focus & Plan, Sleep & Move, Recover & Reflect, Save & Grow, Your Progress) covering all 17 existing tools with zero new screens or storage — every item is the same `setActiveView`/`startDuniyaQuickAction` call the old Deen/Dunya tabs already used. Search filters by label+keywords; an empty result says so rather than showing nothing silently. The old `view-duniya` group browser still exists and works if reached (nothing deleted) but is no longer linked from the main nav since Library already reaches every leaf tool directly.

**Companion Engine extensions** (all inside the existing Next Step Engine, `js/app.js`, same file/module as before — no new `/companion` folder; the project has no build step, so "separate from UI" here means separate *functions*, which was already true): ladder reordered and extended:
- **Day Rescue** (`nsDayRescue`/`nsDayRescueTarget`): evening/before-Maghrib only, only when 4+ real eligible items existed today and 65%+ are still undone — "Forget the full list. Before Maghrib: X. That's enough." One real target picked from the same priority order Home already uses (today's Priority card → first open Plan My Day task → the Deen priority). Verified against real seeded data (4/27 done, 15%): fired with the exact target and phrasing, and its button really completed the task.
- **Sleep-aware wind-down** (`nsPoorSleepWindDown`, `lastNightSleepHours`): real sleep hours from yesterday's journey record, only from 9pm onward in the evening/night stages, only under 6 real hours, never while already asleep. In practice this rarely fires on its own once Isha is done, because the night reflection (already built) takes over first and already avoids suggesting more productivity — both paths reach the same honest outcome, documented rather than hidden.
- **Energy check-in** (`todayEnergy`/`setTodayEnergy`, Home, below the recommendation): Low/Okay/Good, asked not inferred, stored per day. Low energy caps any suggested session at 15 minutes — verified 45→15 min with the same real data, energy as the only variable changed.
- **Learned session length** (`nsLearnedSessionCap`): after 3+ Later/Not-useful responses to a *kind* of session suggestion, future ones from that kind cap at 15 minutes instead of the normal up-to-45 — verified with three real "Not useful" clicks, 45→15 min. This is the one adaptation actually built from the feedback store added last session; nothing else is inferred from it yet (no time-of-day learning, no cross-domain claims) — that needs real history this app doesn't have yet.
- **Already Done** (third response alongside Start/Later/Not Useful, per the brief's feedback-loop section): for the two suggestion kinds that start a timer (today's Priority card, the Dunya study suggestion), directly records the real completion without running a timer — verified it writes to the same `study_focus` records the timer path uses and the Today's Progress ring moves accordingly.
- **Hamdard reads the same context**: "I wasted my day" now calls the same `progressToday()`/`nsDayRescueTarget()` the Life screen uses instead of a fixed line — verified it named the real next open task and real today's count, and its action button actually completed the task and returned to Life. Still fully scripted (a fixed menu of situations), not open-text — rule B4's cloud/local decision and crisis wording still don't exist, so open-text chat still isn't shipped.

**Confirmed already correct, not rebuilt**: Money's avoided-vs-saved distinction ("Avoided is not the same as saved. Only money you add to a goal or keep counts as saved.") already existed from an earlier session and needed no change — verified it's still reachable and intact through Library.

**Not built this pass** (Phase 3+ in the brief's own order, honestly not attempted): a real `UserContextSnapshot`/scored `ActionCandidate` ranking model (the ladder is still first-match, not scored — accurate to how little history exists so far), time-of-day rejection learning (test E's literal "rejects 7pm workouts" scenario — only session-*length* learning was built, not time-of-day), cross-domain pattern claims ("your best study days were also…"), Android usage-based intervention, a dedicated `/companion` `/life` `/data` file split (would need a build step this plain-HTML/JS project doesn't have — logic is already separated into named functions instead).

**Tested** (real stored data, shifted clock, cleared localStorage + IndexedDB backup between fresh-state checks): 3-tab nav and every highlight/back-target; Library search, empty state and routing for Salah/Sleep/Progress/Money; the avatar→Profile→Back round trip from Library; energy check-in storage and its 45→15 min effect; the learned-cap 45→15 min after three real dismissals; Day Rescue's exact trigger condition and real action; Already Done's real store write; Hamdard's context-aware reply and action. Not tested on a phone.
...
**Priority ladder** (first match wins): (1) a focus session already running — shown as "IN PROGRESS" with a live countdown, no buttons, since the Priority card below already controls it; (2) a Salah whose time has arrived and isn't marked — "NOW · Pray X"; (3) a Salah due within `PREP_BUFFER_MIN` (10 min) — informational only, no button, so NURA doesn't suggest starting something with no real room; (4) a fixed Plan My Day commitment starting within 5 minutes — informational; (5) once Isha is done and the day is in Night/Tahajjud, a one-line real reflection (Salah count, study minutes, Qur'an ayahs, today's done/total) instead of any further "do one more thing" suggestion — this sits ahead of steps 6-9 so Home goes calm at night rather than still pushing study; (6) today's Priority card (Study/Fitness/Phone) if it fits before the next event, or a smaller chunk with honest "not quite enough for the full N min" wording if it doesn't; (7) recovery — a fixed commitment whose window already passed and was never marked ("X didn't happen as planned... want a shorter version now?", Do-N-min / Mark-it-done, no shame wording); (8) the Dunya priority (currently Study) sized to the real time left, phrased as "You've studied N min today. Want another M?" when there is already progress; (9) the Deen priority (Qur'an/Hadith/Sunnah) if not done today; (10) an open personal habit; (11) "You're on track today" with the next event and a nudge to rest.

**Session sizing** (`fitSessionMinutes`): real window minus a 10-minute prep buffer, rounded down to 5, floored at 10 minutes, capped at 45 for a single suggested session. Documented and constant — not tuned to match any one example number. Verified against the brief's acceptance scenario: Fajr done, Dhuhr 35 min away, Study priority with 0 minutes logged → "Dhuhr is in 35 minutes. You can fit a 25-minute focused study session before Dhuhr." (25 = 35 − 10, floored to 5). A short chunk uses `startAdhocFocus`, which already records cumulative minutes per session (not the single-priority upsert row), so multiple engine-suggested sessions in a day add up correctly in `todayStudyMinutes()` (sum of every `study_focus` record for today, from any source).

**"Not now"** hides that exact suggestion (`nc_nextstep_snooze`, keyed and timestamped) for 15 minutes and the engine falls through to the next real suggestion in the ladder — never a blank card. A genuine change (a Salah becoming due, a session finishing) is a different key, so it is never held back by an old snooze. Purely informational states (running session, imminent Salah, commitment starting, night reflection) have no "Not now" since there is nothing to dismiss.

**Live updates**: `startDayTimer`'s existing 1-second tick now also calls `renderNextStep()` — every second while a focus session is running (so its countdown moves), otherwise every 15 seconds (enough to notice a prep window opening or a snooze expiring) without rebuilding the whole day flow. Today's Priority card, the Salah/Sunnah/Qur'an flow, Plan My Day, habits, Progress and Profile are all unchanged — the engine only reads them.

**Tested** (real stored data, shifted clock, cleared localStorage + IndexedDB backup between runs to avoid the app's own resilience-restore masking a "fresh" state): the exact acceptance scenario end to end, including reload after completing a session; the imminent-Salah info-only state; "Not now" falling through to the next suggestion and storing the snooze; a missed fixed commitment producing a Recover suggestion, both actions working; the night reflection appearing only once Isha is marked and yielding to a still-pending Isha. Not tested on a phone.


## 2026-09-21 — Clean V1: five tabs, Today priorities + Quick log, event model (Phase 1)

Full old→new mapping: `docs/feature-map.md`. Summary of decisions:

- **Navigation** is Today · Deen · Dunya · Progress · Hamdard. "More" is gone; Profile/settings are behind the avatar in every header (Back returns to the screen you came from). Internal view ids (`sunnah`, `chat`, `duniya`, `more`) were kept so no saved data or code path broke; only labels, nav targets and layout changed.
- **Deen** = Salah (new panel: next Salah, today's five with real completions, edit times, this week from saved completions), Qur'an (unchanged), Practice (Sunnah · Akhlaq · Duas, with a "Sunnah Today" card picked deterministically per day from entries already in NURA, each with its source), Learn (Hadith: Save, "How will I apply this today?" in the user's own words, learning summary). No new religious content was written.
- **Dunya** = four groups (Focus & Study, Health & Energy, Discipline & Mind, Future & Skills), each opening its tools. The Quick Actions grid, the duplicate Today card and the second Recovery entry (inside Habits) were removed: one feature, one place. The group cards show one real stat where one exists (open tasks, last sleep, habits done).
- **Progress** replaces the weekly-report pop-up: Today (the exact item list behind the %), Week (saved daily summaries + category rollups), Patterns ("Not enough data yet" until 14 active days, then real counts; pattern detection itself is Phase 3).
- **Today**: "Today's priorities" (one Deen, one Dunya, one optional personal row) chosen only from real data: earliest unmarked prayer whose time has come → else the Deen priority chosen at onboarding → else the next prayer; today's Priority/first open user task → else the Dunya priority action; first open habit. **Quick log** (Salah, Qur'an, Hadith, Study, Workout, Sleep/wake, Money, Urge, Mood, Habit) writes through the same stores as the rest of the app. A mood check-in is context and does not count as a completed action.
- **Onboarding** now asks name → one Deen priority → one Dunya priority → prayer times; each answer is saved at once (`deenPriority`, `dunyaPriority`, `onboardingStep`), skipping is allowed, it resumes at the missing step, and users who were already onboarded are never asked again (their priorities stay unset until they choose them under Profile → Change my priorities).
- **Event model** (`nc_events`, `window.NuraEvents`): `{id, type, timestamp, date, metadata, source, duration?, value?, recordId?}`. Every ProgressStore record/remove is mirrored automatically (undoing a tick removes its event); sleep start, wake, urge and recovery success are logged directly; existing history was back-filled once (`nc_events_backfilled_v1`). Types: SALAH_COMPLETED, QURAN_READ, HADITH_READ, STUDY_STARTED, STUDY_COMPLETED, SLEEP_STARTED, WAKE_UP, WORKOUT_COMPLETED, HABIT_COMPLETED, URGE_LOGGED, RECOVERY_SUCCESS, SMOKING_LOGGED, MONEY_SAVED, APP_USAGE_SUMMARY (reserved for Phase 4, never emitted), PLAN_TASK_COMPLETED, MOOD_LOGGED, ACTION_COMPLETED. Today progress and reports still read the stores; the future pattern engine will read events. Local only.
- **Privacy screen** "How NURA Understands Me": what data is used, what is not used, phone-usage permission status, guidance level (Low/Balanced/High, stored, unused until suggestions exist), patterns detected, and "Delete what NURA learned about me" (events + learned patterns; progress history, habits and profile stay).
- **Removed from the UI**: the coins card (gamification). Its data is untouched.

**Not built** (deliberately): pattern calculation, Personal Pattern Map, recommendations, notifications, Android usage signals, Hamdard context. Tested in a browser only; not on a phone.
...
**Home layout.** Status card is now "NEXT SALAH — name, time, live `1h 24m 18s remaining`" plus the five times. Timeline blocks: Fajr, Morning, Dhuhr, Afternoon, Asr, Evening, Maghrib, Isha, Night, Tahajjud (optional). Prayer blocks start at the prayer time and last 45 minutes (display grouping, not a ruling); period blocks fill the time until the next prayer. Every block is expandable: header shows "2 of 4 completed" or "All done", the detail shows each item. Skipped items are excluded from the count and shown as "Skipped for today" with Undo. A prayer block hands the day to the next block as soon as every step is done or skipped.

**Salah flow.** Fard (locked until its time, "Mark done") → then Sunnah → Tasbihat → Qur'an appear. Sunnah rows come only from existing Sunnah-log items (Fajr before; Dhuhr before/after; Isha → Witr); Asr and Maghrib have none in NURA, so their flow goes straight to Tasbihat. Every step after Fard can be skipped, no guilt wording.

**Tasbihat.** Inline counter (SubhanAllah 33 → Alhamdulillah 33 → Allahu Akbar 34, +, −1, Reset, or tick to complete). Counts in `nc_salah_flow`; completing also ticks that prayer's "Dhikr after salah" in the Sunnah log so the Sunnah page and weekly report agree. NOTE for the content reviewer: the Sunnah page's existing after-Salah item cites Sahih Muslim 597a with 33/33/33 + tahlil, whereas this counter follows the requested 33/33/34. Both patterns exist in the literature; a named reviewer should confirm which NURA presents.

**Qur'an.** "Read Qur'an" opens the real Qur'an page (Sunnah → Quran) with a "← Back to Home" bar; "Continue" opens the last surah scrolled to the last ayah. Reading is recorded only when an ayah has stayed on screen for 3 seconds (`nc_quran_progress`: last position + ayahs read per day) and feeds ProgressStore as "Qur'an reading: N ayahs" (Today + Weekly Report). If the user opened it from a prayer's flow, that step ticks itself after the first counted ayah; it can also be ticked by hand for a paper Qur'an.

**User tasks only.** Morning/Afternoon/Evening/Night show only Plan My Day activities the user made, placed by time; a "+ Add task" (name + optional time) on Home writes a normal Plan My Day activity (`homeBlock` remembers the block; tasks made on Home can be removed with ×). NURA adds no personal tasks; Sunnah suggestions (Morning Adhkar, Evening Adhkar, before-sleep) sit under a separate "Sunnah for now" heading and are tickable but are not scheduled tasks. Duha is no longer shown on Home.

**Sunnah page** is regrouped into Morning / Salah / Daily life / Character (Akhlaq) / Evening & night using only entries already in NURA. Daily life is empty on purpose (eating, drinking, dressing, entering/leaving home, cleanliness, dealing with people) and Ikhlas/Falaq/Nas bedtime and masjid manners are not present: sunnah.com could not be reached from this environment to verify them, so none were added. They need source + named reviewer first. Home rows open the matching Sunnah section ("Open ›").

**Tested** (browser, shifted clock): validation, save, edit, reload persistence, Next Salah rollover Dhuhr→Asr, full Fajr flow with tasbih 100 taps, Qur'an round trip (dwell counting, Continue, return), custom task add/complete/remove, Sunnah deep link, 1 AM and next-Fajr day change, weekly report rows. Not tested on a phone; Android "suggest from location" not wired to native permission.
